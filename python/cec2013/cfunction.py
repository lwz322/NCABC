...
class CFunction(object):
    ...
    def get_lbound(self, ivar):
        assert (ivar >= 0 and ivar < self.__dim_), [
            "ivar is not in valid variable range: %d not in [0,%d]" % ivar, self.__dim_]
    ...
    def __evaluate_inner_(self, x):
        ...
        for i in range(self.__nofunc_):
            tmpsum[i] = self.__weight_[
                i] * (self.__C_ * self.__fi_[i] / self.__fmaxi_[i] + self.__bias_[i])
    ...
    def __calculate_weights(self, x):
        ...
        for i in range(self.__nofunc_):
            ...
            self.__weight_[
                i] = np.exp(-mysum/(2.0 * self.__dim_ * self.__sigma_[i] * self.__sigma_[i]))
