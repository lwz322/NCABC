...
function r3pso(func_no, runs, bounds, optima, tolerance, Particle_Number, Max_Gen, t_dis)
    ...
    % for i=1:ps
    %     if i==1
    %         if pbestval(ps)>nbestval(i)
    %             nbestval(i)=pbestval(ps);
    %             nbest(i)=pbest(ps);
    %         end
    ...
    for i = 2:me
        ...
        for j = 1:ps
            ...
            if j == 1
                ...
                if pbestval(ps) > nbestval(j)
                    nbestval(j) = pbestval(ps);
                    nbest(j) = pbest(ps);
                end
    ...
    dlmwrite(strcat('r3pso_info', char(num2str(func_no)), '_', char(num2str(runs)), '.txt'), traceInfo, 'newline', 'pc');
    dlmwrite(strcat('r3pso_result', char(num2str(func_no)), '_', char(num2str(runs)), '.txt'), fresult, 'newline', 'pc');
    dlmwrite(strcat('r3pso_peak', char(num2str(func_no)), '_', char(num2str(runs)), '.txt'), finalpeak, 'newline', 'pc');
    clear all
